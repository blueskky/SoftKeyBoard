...
// Vars
var _wrs_currentPath = "http://kc.t.tianyuyun.cn/coursecenter/open/";
var _wrs_editMode;
 ...
 */
function wrs_addElementEvents(target, doubleClickHandler, mousedownHandler, mouseupHandler, clickHandler) {
	...
	if (clickHandler) {
	    wrs_addEvent(target, "click", function (event) {
	        var realEvent = (event) ? event : window.event;
	        var element = realEvent.srcElement ? realEvent.srcElement : realEvent.target;
	        clickHandler(target, element, realEvent);
	    });
	}
}
 ...
 */
function wrs_addIframeEvents(iframe, doubleClickHandler, mousedownHandler, mouseupHandler, clickHandler) {
	wrs_addElementEvents(iframe.contentWindow.document, function (target, element, event) {
		    ...
		    clickHandler(iframe, element, event);
		}
	);
}
	...
	try {
		...
		if (_wrs_isNewElement) {
			if (document.selection) {
                ...
                // Fix to set the caret after the inserted image via CefGlue browser - Ldj.2016.09.09
				var editor = CKEDITOR.instances[focusElement.id];
				var ckNode = new CKEDITOR.dom.node(element);
				var ckRange = editor.createRange();
				ckRange.moveToPosition(ckNode, CKEDITOR.POSITION_AFTER_END);
				ckRange.select();
				//var img = '<img src="' + element.src + '" style="max-width:100%;" />';
				//CKEDITOR.instances[focusElement.id].insertHtml(img);
			}
		}
